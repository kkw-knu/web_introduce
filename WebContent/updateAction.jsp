...
<body>
	...
	//하나의 입력값이라도 null값이 입력되었을 때
		if(request.getParameter("infoTitle") ==null || request.getParameter("infoContent") == null){
			PrintWriter script = response.getWriter();
			script.println("<script>");
			...
			if(!request.getParameter("userPW").equals(i.getUserPW())){
				...
				int result = infoDAO.update(infoID, request.getParameter("infoTitle"), request.getParameter("infoContent"));
				if(result == -1){
					PrintWriter script = response.getWriter();
					script.println("<script>");
